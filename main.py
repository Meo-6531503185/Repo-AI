...
def get_installation_token():
        ...
        # st.write("Installations API Response:", response.status_code, response.json()) 

        # if not response or "id" not in response[0]:  
        #     raise Exception("No installations found. Ensure the GitHub App is installed.")

        # installation_id = response[0]["id"]
        # if response.status_code != 200:
        #     raise Exception(f"Failed to fetch installations: {response.json()}")

        installations = response.json()
        # st.write("Installations Response:", installations)
...
def reasoning_user_questions(user_question):
    system_template = """
    ...
    try:
        ...
        if "Provide Repository Overview" in explanation:
            ...
            response=Enhance_full_project(user_question,st.session_state.repo_data)
...
def Enhance_full_project(user_question, repo_data):
    system_template = """
    Perform a comprehensive enhancement of the entire project based on the given repository data. 
    Analyze and refactor the code while maintaining all existing functionality. Follow a structured approach:
    
    1️ **Codebase Analysis**:
       - Identify code duplication, inefficiencies, and areas violating best practices.
    ...
    6️ **Final Verification & Recommendations**:
       ...
       - Ensure the project builds successfully and aligns with best practices.
    """
    
    # Pass `repo_data` as part of the user input
    prompt_template = ChatPromptTemplate.from_messages([
        ("system", system_template),
        ("user", "Repository Details:\n{repo_data}\n\nQuestion: {text}")
    ])
    
    # Pass both `repo_data` and `text` when invoking the prompt
    prompt = prompt_template.invoke({"text": user_question, "repo_data": repo_data})
...
def main():
    ...
    if user_question:
        ...
        if detect_push_command(user_question):
            response = reasoning_user_questions(user_question)
    ...
    for sender, message in st.session_state.chat_history:
        with st.chat_message("user" if sender == "User" else "assistant"):
            # Check if the response is a code snippet (simple heuristic)
            if sender == "AI" and "public class" in message:  # Add more conditions if needed
                st.code(message, language="java")  # Show as Java code
            else:
