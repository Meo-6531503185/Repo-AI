...
if 'final_test_case' not in st.session_state:
    st.session_state.final_test_case = None
...
def fetch_file_content(file_url):
    ...
    if response.status_code == 200:
        ...
        # st.write(f"Fetched file content type: {type(content)}") # Debugging
        # print(f"Fetched file content (first 200 chars): {content[:200]}")  # Debugging
        return content
    else:
...
def get_text_chunks(extracted_text,max_tokens = 20000, overlap = 200):
    ...
    # java_docs = python_splitter.split_text(extracted_text)
    # return java_docs
...
"""
    ...
    with open(os.path.join(output_dir, f"{test_class_name}.java"), "w") as file:
        ...
        st.write("Test case generated successfully.")
    return test_case
...
def handle_refactor_and_test(user_question):
    ...
    if file_name:
            ...
            if matching_files:
                ...
                st.session_state.final_code = refactor_code_with_ai(file_content, user_question, reasons="")
                st.session_state.final_test_case  = generate_java_code_and_test(st.session_state.final_code)
                while True:
                    ...
                    if test_code == 0:
                        ...
                        return st.session_state.final_code, st.session_state.final_test_case
                    else:
                        print("Tests failed. Analyzing reasons...")
                        analyze_test_failures(test_output, st.session_state.final_code, user_question)
            else:
...
,
   ...
   github_app_private_key = github_private_file
    ...
    wrapper.create_branch("your_code_branch")
    # Define file details
    ...
    try:
        ...
        if "Provide Repository Overview" in explanation:
            ...
            if detect_push_command(user_question):
                ...
                push_refactored_code_to_github(response, test_case, full_repo_name)
...
def main():
    ...
    if user_question:
       ...
       if detect_push_command(user_question):
            if st.session_state.final_code and st.session_state.final_test_case:
                push_refactored_code_to_github(
                st.session_state.final_code,
                st.session_state.final_test_case,
                full_repo_name
                )
            else:
